...
class Trainer(object):
    ...
    def criterion(self):
        if self._wrapped_criterion is None:
            if utils.has_parameters(self._criterion) and self.data_parallel_world_size > 1 and (not self.cfg.optimization.use_bmuf) and (not self.tpu):
    ...
    def model(self):
        if self._wrapped_model is None:
            if self.data_parallel_world_size > 1 and (not self.cfg.optimization.use_bmuf) and (not self.tpu):
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            if not self.cfg.optimization.use_bmuf and self.cfg.distributed_training.distributed_wrapper != 'SlowMo' and (not self.tpu):
