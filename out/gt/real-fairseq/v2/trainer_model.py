...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        if self.args.zero_sharding == 'os':
            if (self.args.fp16 and (not self.args.memory_efficient_fp16) and (not self.args.memory_efficient_bf16)) and (not self.args.fp16_no_flatten_grads):
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            with torch.autograd.profiler.record_function('optimizer'):
                self.optimizer.step()
                print('LOG STMT: Model Parameters = %s' % str(self.model.parameters))
        ...
        if not overflow or self.args.distributed_wrapper == 'SlowMo':
            ...
            if self.tpu:
                ...
                logging_output = self._reduce_and_log_stats(logging_outputs, sample_size, grad_norm)
                if self.cuda and self.args.empty_cache_freq > 0 and ((self.get_num_updates() + self.args.empty_cache_freq - 1) % self.args.empty_cache_freq == 0):
                    torch.cuda.empty_cache()
        if self.args.fp16:
    ...
    def _sync_stats(self):
        if self.data_parallel_world_size == 1:
            ...
            return (self.get_num_updates() + 1) % self.args.global_sync_iter == 0 and self.get_num_updates() + 1 > self.args.warmup_iterations
        else:
            return True
    ...
    def _all_gather_list_sync(self, logging_outputs: List[Dict[str, Any]], *extra_stats_to_sum, ignore=False):
        ...
        if ignore:
            logging_outputs = []
        results = list(zip(*distributed_utils.all_gather_list([logging_outputs] + list(extra_stats_to_sum), max_size=getattr(self.args, 'all_gather_list_size', 16384), group=self.data_parallel_process_group)))
    ...
    def _check_grad_norms(self, grad_norm):
        ...
        if self._grad_norm_buf is not None:
            ...
            def is_consistent(tensor):
                ...
                return not torch.isfinite(tensor).any() or (max_abs_diff / (tensor[0] + 1e-06) < 1e-06).all()
            if not is_consistent(self._grad_norm_buf):
                pretty_detail = '\n'.join(('rank {:3d} = {:.8f}'.format(r, n) for (r, n) in enumerate(self._grad_norm_buf.tolist())))
                error_detail = 'grad_norm across the workers:\n{}\n'.format(pretty_detail)
                raise RuntimeError('Fatal error: gradients are inconsistent between workers. Try --ddp-backend=no_c10d. Or are you mixing up different generation of GPUs in training?' + '\n' + '-' * 80 + '\n{}\n'.format(error_detail) + '-' * 80)
    ...
    def _reduce_and_log_stats(self, logging_outputs, sample_size, grad_norm=None):
        if grad_norm is not None:
            ...
            if self.args.clip_norm > 0:
                metrics.log_scalar('clip', torch.where(grad_norm > self.args.clip_norm, grad_norm.new_tensor(100), grad_norm.new_tensor(0)), priority=500, round=1)
    ...
    def _check_xla_compilation(self, message=None):
        ...
        if num_xla_compiles > self._num_xla_compiles:
            if message is None:
                message = 'too many of these can lead to slow training, but we expect a few in the beginning'
