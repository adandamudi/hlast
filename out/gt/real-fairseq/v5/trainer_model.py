...
class Trainer(object):
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        if self.is_data_parallel_master:
            logger.info(f'Preparing to save checkpoint to {filename} after {self.get_num_updates()} updates')
    ...
    def load_checkpoint(self, filename, reset_optimizer=False, reset_lr_scheduler=False, optimizer_overrides=None, reset_meters=False):
        ...
        if bexists:
            ...
            if load_on_all_ranks or self.data_parallel_rank == 0:
                ...
                if not load_on_all_ranks and self.cfg.distributed_training.zero_sharding == 'os' and ('last_optimizer_state' in state) and (self.data_parallel_world_size > 1):
